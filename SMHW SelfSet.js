...
function updateHwk(){
  ...
  var hwkData = JSON.parse(localStorage["SMHW-SelfSet"]);
  if (hwkData.length === 1){
    target.innerHTML = hwkData.length + " piece of self-set homework. <br>";
  } else {
    if  (hwkData.length === 0) {
      target.innerHTML = "No Self-Set Homework";
    } else {
      target.innerHTML = hwkData.length + " pieces of self-set homework. <br>";
    }
  }
  var num = 0;
  for (var i in localStorage) {
      var currentHwkData = JSON.parse(hwkData[num]);
      var hwkSubject = currentHwkData[0];
      var hwkTitle = currentHwkData[1];
      var hwkDue = currentHwkData[2];
      var hwkDesc = currentHwkData[3];
      num = num + 1;
